...
namespace Avalon.Raft.Core
{
    ...
    public class VolatileLeaderState
    {
        ...
        /// </summary>
        internal ConcurrentDictionary<Guid, long> NextIndices { get; } = new ConcurrentDictionary<Guid, long>();

        /// <summary>
        /// for each server, index of highest log entry known to be replicated on server (initialized to 0, increases monotonically)
        /// </summary>
        internal ConcurrentDictionary<Guid, long> MatchIndices { get; } = new ConcurrentDictionary<Guid, long>();
